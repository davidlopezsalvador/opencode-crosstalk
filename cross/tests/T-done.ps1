# Cross done tests (Phase 2, idempotency state).
# Usage: powershell -NoProfile -ExecutionPolicy Bypass -File tests\T-done.ps1
...
Write-Host "== T-done: claim -> done by owner =="
$m1 = 'msg_tdone_1'
Reset-State
...
Write-Host "== T-done: done without prior claim =="
$m2 = 'msg_tdone_2'
Reset-State
$r = Get-Json (Invoke-CrossCli @('done', '--state-file', $script:StateFile, '--msg', $m2, '--owner', $A))
Assert-True (-not $r.ok -and $r.code -eq 2) 'done without claim -> code 2' $r.code
Assert-True ($r.err -eq 'NOT_CLAIMED') 'err=NOT_CLAIMED' $r.err

Write-Host "== T-done: done on SUPERSEDED msg =="
$m3 = 'msg_tdone_3'
Reset-State
...
Assert-True (-not $r.ok -and $r.err -eq 'RELEASED_CANNOT_DONE') 'done on SUPERSEDED -> RELEASED_CANNOT_DONE' $r.err

Write-Host "== T-done: done by non-owner =="
$m4 = 'msg_tdone_4'
Reset-State
...
Assert-True (-not $r.ok -and $r.err -eq 'NOT_OWNER') 'done non-owner -> NOT_OWNER' $r.err
