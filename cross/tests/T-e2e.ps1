# Tests E2E reales (Fase 3/D4): transporte real contra el servidor OpenCode Desktop.
# La entrega va a la sesion del lider (my_session_id) como destino; el ACK/NACK se
# inyecta en la sesion del lider tal como lo haria un receptor real (7.1/7.5).
# Uso: powershell -NoProfile -ExecutionPolicy Bypass -File tests\T-e2e.ps1
...
if (-not $port -or -not $password) { throw 'No se pudieron detectar credenciales del servidor' }
...
Write-Host "== T-e2e: happy ACK (transporte real) =="
$tokH = 'E2E-' + [System.Guid]::NewGuid().ToString('N').Substring(0, 8)
Write-OutboxEntry 'e2e_happy' $mySession $tokH
$job = Invoke-SendCli 'e2e_happy' $mySession $tokH $script:Outbox 'TEST E2E CROSS - no requiere respuesta' 45
...
Assert-True ($codeAck -eq '204') 'ACK inyectado -> HTTP 204' $codeAck
$j = $null
try { $j = ($raw | ConvertFrom-Json) } catch { }
Assert-True ($null -ne $j -and $j.ok -and $j.outbox_state -eq 'CONFIRMADO' -and $j.ack) 'E2E happy -> CONFIRMADO via transporte real' $raw
Assert-True ($null -ne $j -and $j.ack_id -eq $mySession -and $j.ack_model -eq $myModel) 'E2E ack_id/ack_model correctos' "$($j.ack_id)|$($j.ack_model)"

Write-Host "== T-e2e: NACK (transporte real) =="
$tokN = 'E2E-' + [System.Guid]::NewGuid().ToString('N').Substring(0, 8)
Write-OutboxEntry 'e2e_nack' $mySession $tokN
$job = Invoke-SendCli 'e2e_nack' $mySession $tokN $script:Outbox 'TEST E2E CROSS NACK - no requiere respuesta' 45
$codeNack = Invoke-CrossPost $mySession "NACK:${tokN}:${mySession}:${myModel}:CAPACITY"
Start-Sleep -Seconds 1
$null = Wait-Job $job -Timeout 70 -ErrorAction SilentlyContinue
$raw = Receive-Job $job
Remove-Job $job -Force
Assert-True ($codeNack -eq '204') 'NACK inyectado -> HTTP 204' $codeNack
$j = $null
try { $j = ($raw | ConvertFrom-Json) } catch { }
Assert-True ($null -ne $j -and -not $j.ok -and $j.outbox_state -eq 'NACKED' -and $j.reason -eq 'CAPACITY') 'E2E NACK -> NACKED con razon' $raw

Write-Host "== T-e2e: 404 destino inexistente =="
...
Write-Host "== T-e2e: observabilidad (delivery_log + audit_log) =="
$hasLog = $false
...
Assert-True $hasLog 'delivery_log.jsonl contiene e2e_happy CONFIRMADO' $deliveryLog
$audit = Join-Path $whiteboard 'audit_log.md'
$hasAudit = @(Get-Content -LiteralPath $audit | Where-Object { $_ -match 'e2e_happy' }).Count -ge 1
Assert-True $hasAudit 'audit_log.md registra e2e_happy' ''
