...
function Send-CrossAck {
    ...
    if (-not $Token) { return @{ ok = $false; err = 'USAGE_ERROR'; detail = 'falta --token' } }
    if (-not $ForMsgId) { return @{ ok = $false; err = 'USAGE_ERROR'; detail = 'falta --for-msg-id' } }
    $myId = Get-CrossMyId
...
function Send-CrossNack {
    ...
    if ($Reason -notin $validReasons) {
        return @{ ok = $false; err = 'NACK_REASON_INVALID'; detail = "razon no cerrada: $Reason (validas: $($validReasons -join ', '))" }
    }
    $myId = Get-CrossMyId
    if (-not $Dest) { $Dest = $myId }
    if (-not $Model) { $Model = Get-CrossMyModel }
    ...
    if ($ForRunId) {
        if (-not $Model) {
            return @{ ok = $false; err = 'USAGE_ERROR'; detail = '--for-run-id requiere --model o config.my_model (formato enriquecido de 7 segmentos)' }
        }
        $segs += $ForMsgId
        $segs += $ForRunId
    }
...
function Send-CrossResume {
    ...
    if (-not $To) { return @{ ok = $false; err = 'USAGE_ERROR'; detail = 'falta --to ses_X' } }
    if (-not $TaskId) { return @{ ok = $false; err = 'USAGE_ERROR'; detail = 'falta --task-id' } }
    if (-not $Text) {
        $base = if ($From) { "Continúa desde `"$From`"." } else { 'Continúa tu trabajo.' }
        $Text = "$base Completa la tarea $TaskId y entrega el informe final firmado."
    }
    $sent = $false
    if ($SendFn) {
        [void](& $SendFn $To $Text)
    ...
    $nota = 'resume (NO nuevo msg_id, NO incrementa attempt)'
    if ($From) { $nota += "; desde $From" }
...
function Restart-CrossTask {
    ...
    if (-not $MsgId) { return @{ ok = $false; err = 'USAGE_ERROR'; detail = 'falta --msg-id' } }
...
function Send-CrossNudge {
    ...
    if (-not $To) { return @{ ok = $false; err = 'USAGE_ERROR'; detail = 'falta --to ses_X' } }
    if (-not $Task) { return @{ ok = $false; err = 'USAGE_ERROR'; detail = 'falta --task' } }
    $text = "NUDGE. Ignora 'Continue' u orientaciones previas. Completa AHORA la tarea: $Task"
...
function Write-CrossEscalated {
    ...
    if (-not $MsgId) { return @{ ok = $false; err = 'USAGE_ERROR'; detail = 'falta --msg-id' } }
    if (-not $To) { return @{ ok = $false; err = 'USAGE_ERROR'; detail = 'falta --to ses_Y' } }
    if (-not $Reason) { return @{ ok = $false; err = 'USAGE_ERROR'; detail = 'falta --reason' } }
...
function Write-CrossDlq {
    ...
    if ($Flag -notin $validFlags) {
        return @{ ok = $false; err = 'DLQ_FLAG_INVALID'; detail = "flag no cerrado: $Flag (validos: $($validFlags -join ', '))" }
    }
    if (-not $OutboxPath) { $OutboxPath = (Get-CrossActionPaths).outbox }
    ...
    $audit = Write-AuditEntry -MsgId $MsgId -Dest $To -Tipo 'DLQ' -Estado 'ESCRITO' -Nota "flag=$Flag; reintentos=$Retries" -AuditPath $AuditPath
...
function Get-CrossDiagnose {
    ...
    if (-not $Msg) { return @{ ok = $false; err = 'USAGE_ERROR'; detail = 'falta --msg' } }
    if (-not $OutboxPath) { $OutboxPath = (Get-CrossActionPaths).outbox }
    ...
    $action = 'sin lineas del destino en la ventana: agente dormido o session distinta'
    if ($matched.Count -gt 0) {
        if ($joined -match 'model not found|model_not_found|no such model|invalid model|not a valid model|unauthori[sz]ed|invalid api key|invalid_api_key|api.?key|authentication failed|\b401\b|\b403\b') {
            ...
            $action = 'error de configuracion (auth/modelo): reportar al humano en DLQ'
            ...
            $action = 'fallo del proveedor, agente sano: renovar lease y esperar (12.10)'
